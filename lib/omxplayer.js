...
OMXPlayer.prototype.start = function(moviePath, options, callback) {
	...
	this.stop(function(error) {
		...
		if (configuration.omxPlayerPath) {
			...
			self._log("Execute  '" + configuration.omxPlayerPath + "' parameters=", parameters.join(" "), " spawnOptions=",
					options.spawnOptions);

			// var omxProcess = child_process.spawn(configuration.omxPlayerPath, parameters, options.spawnOptions);
			self._log(configuration.omxPlayerPath + " " + parameters.join(" "));
			var omxProcess = child_process.exec(configuration.omxPlayerPath + " " + parameters.join(" "), function (error, stdout, stderr) {
				console.log(stdout);
				console.log(stderr);
				console.log(error);
				self.emit('exit');
